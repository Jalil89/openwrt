...
struct list_head flow_list; // a list of flows
struct flow_info fli;
...
void ath_init(void){
	...
	fli.avg_pkt_size   = 0;
	fli.total_pkt      = 0;
	fli.total_pkt_succ = 0;
}

/*
 * @author Jalil
 * the functions to retrive and register queue infos
 */
...
void ath_get_flow_info(struct flow_info *fi, unsigned int dst_addr){
	...
	fi->dest_addr = fli.dest_addr;
	fi->avg_pkt_size = fli.avg_pkt_size;
	fi->total_pkt = fli.total_pkt;
	fi->total_pkt_succ = fli.total_pkt_succ;
}
