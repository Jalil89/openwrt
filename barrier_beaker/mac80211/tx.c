...
netdev_tx_t ieee80211_subif_start_xmit_frags(struct sk_buff *skb,struct net_device *dev);
	...
	if (iph && iph->protocol &&(iph->protocol == IPPROTO_UDP)) {
		...
		if (IS_TS(tsh)){
			...
			while(bitmap_has_next(&b)){
				...
				ptr_payload = payload + ETH_HLEN + IP_HDR_LEN + UDP_HDR_LEN + OVERHEAD_LEN;
...
netdev_tx_t ieee80211_existing_tx(struct sk_buff *skb,
	...
	struct tshdr *tsh;
	u8 *payload ,*temp;
	unsigned int data_len,frag_len, opt_frag_size, curr_pos;
	curr_pos = 0;

	// length of data accupied by only TS packets
	unsigned int payload_len, IP_PSIZE;
	...
	if (iph && iph->protocol &&(iph->protocol == IPPROTO_UDP)) {
		...
		if (IS_TS(tsh)){
			...
			while (opt_frag_size <= IP_PSIZE){
				// set up RTP number for deaggregation at the decoder
				tsh->rtpsqnum = (tsh->rtpsqnum*BITMAP_MAX_SIZE + ite) % 65535;
				...
				//advance to the payload (TS packet)
				ptr_temp = ptr_temp + RTP_LEN;
				ptr_payload = ptr_payload + RTP_LEN + ite*opt_frag_size;
				...
				new_skb->priority = IEEE80211_AC_BK;
